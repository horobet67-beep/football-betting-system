"""
Romanian League data adapter for the v2 system.
Converts Romanian CSV format to standardized match data format.
"""
import pandas as pd
import numpy as np
...
class RomanianDataAdapter:
    """
    Adapter for Romanian league CSV data format.
    Converts to standardized format expected by the v2 system.
    """
    
    # Column mapping from Romanian format to standard format
    COLUMN_MAPPING = {
        # Basic match info
        'date_GMT': 'Date',
    ...
    def __init__(self, data_dir: str):
        ...
        Args:
            data_dir: Path to directory containing Romanian CSV files
        """
    ...
    def load_season_data(self, season_files: List[str]) -> pd.DataFrame:
        ...
        for filename in season_files:
            filepath = self.data_dir / "liga1-romania" / filename
            if filepath.exists():
    ...
    def _convert_format(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for romanian_col, standard_col in self.COLUMN_MAPPING.items():
            if romanian_col in df.columns:
                converted[standard_col] = df[romanian_col]
        ...
        # Add derived features from Romanian data
    ...
    def _add_derived_features(self, converted_df: pd.DataFrame, original_df: pd.DataFrame) -> None:
        """Add derived features from Romanian-specific columns."""
...
def load_romanian_data(
    ...
    """
    Convenience function to load Romanian league data.
    ...
    adapter = RomanianDataAdapter(data_dir)
