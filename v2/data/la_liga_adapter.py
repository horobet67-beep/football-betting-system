"""
La Liga (Spain) data adapter for the v2 system.
Converts La Liga CSV format to standardized match data format.
"""
import pandas as pd
import numpy as np
...
class LaLigaDataAdapter:
    """
    Adapter for La Liga CSV data format.
    Converts to standardized format expected by the v2 system.
    Same format as Bundesliga/Romanian data - uses identical column structure.
    """
    
    # Column mapping from La Liga format to standard format
    COLUMN_MAPPING = {
        # Basic match info
        'date_GMT': 'Date',
    ...
    def __init__(self, data_dir: str):
        ...
        Args:
            data_dir: Path to directory containing La Liga CSV files
        """
    ...
    def load_season_data(self, season_files: List[str]) -> pd.DataFrame:
        ...
        for filename in season_files:
            filepath = self.data_dir / "la_liga" / filename
            if filepath.exists():
    ...
    def _convert_format(self, df: pd.DataFrame) -> pd.DataFrame:
        """Convert La Liga format to standard format."""
        converted = pd.DataFrame()
        
        # Map columns that exist
        for la_liga_col, standard_col in self.COLUMN_MAPPING.items():
            if la_liga_col in df.columns:
                converted[standard_col] = df[la_liga_col]
        ...
        # Add derived features from La Liga data
    ...
    def _add_derived_features(self, converted_df: pd.DataFrame, original_df: pd.DataFrame) -> None:
        """Add derived features from La Liga-specific columns."""
...
def load_la_liga_data(
    ...
    """
    if seasons is None:
        # Load all available seasons (adjust filenames as needed)
        seasons = [
    ...
    adapter = LaLigaDataAdapter(data_dir)
