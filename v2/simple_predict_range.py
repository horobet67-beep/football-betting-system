...
class SimpleRomanianPredictor:
    ...
    def __init__(self):
        ...
        self.confidence_thresholds = {
            ...
            'total_over_8_5_corners': 0.65,  # 74.2% win rate, +21.9 units (90 days) ⭐
            'home_over_1_5_cards': 0.75,     # 66.7% win rate, +0.4 units (90 days) ✅
            ...
            'both_teams_to_score': 0.99,         # 29.2% win rate, -10 units 💀 DISABLED
            'away_over_1_5_goals': 0.99,         # 0% win rate, -4 units 💀 DISABLED
            'total_over_3_5_goals': 0.99,        # 0% win rate, -1 unit 💀 DISABLED
...
def predict_date_range_matches(start_date: str, end_date: str):
    ...
    if len(future_matches) == 0:
        print("\n⚠️ No future matches found in date range")
        return
    
    print(f"\n📊 Analysis: {len(historical_data)} historical, {len(future_matches)} future matches")
    ...
    if bet_recommendations:
        for i, match in enumerate(bet_recommendations, 1):
            ...
            print(f"   🎯 Adaptive Threshold: {bet.threshold:.1%}")
    ...
    print(f"✅ BET recommendations: {bets} ({bets/total*100:.1f}%)")
    print(f"❌ NO BET: {total-bets} ({(total-bets)/total*100:.1f}%)")
    ...
    print(f"\n🚀 Enhanced with form weighting & adaptive thresholds!")
