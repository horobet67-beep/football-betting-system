"""
Generate predictions for upcoming Premier League matches.
Uses optimized Extreme Recent weights (40/30/15/10/5) - 85.4% WR
"""
...
def predict_premier_league_matches(start_date=None, end_date=None, days_ahead=7):
    """
    Generate predictions for upcoming Premier League matches.
    ...
    print("\n" + "="*80)
    print("🔮 Premier League Upcoming Match Predictions")
    print("="*80)
    ...
    print(f"⚙️  Using: EXTREME RECENT weights (40/30/15/10/5) - 85.4% WR")
    
    # Load all data including future matches
    print(f"\n{'='*80}")
    print("Loading upcoming fixtures...")
    all_data = load_premier_league_data(include_future=True)
    ...
    if bet_recommendations:
        ...
        print(f"   • These are based on BACKTESTED performance (85.4% WR)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Predict upcoming Premier League matches')
    ...
    # Generate predictions
    try:
        predict_premier_league_matches(
